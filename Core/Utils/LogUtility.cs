...
namespace Core.Utils
{
    public class LogUtility
    {
        ...
        private void SaveErrorLogInTextFile(LogType logType, HttpContext httpContext, Exception ex, string loginId, string errorSource, string apiParameters)
        {
            try
            {
        ...
        public void WriteErrorLog(HttpContext httpContext, Exception ex, string loginId, string errorSource, string apiParameters)
        {
            try
            {
        ...
        public void WriteEventErrorLog(Exception ex, string loginId, string errorSource, string apiParameters)
        {
            try
            {
                DelegateSaveErrorLog dlgt = new DelegateSaveErrorLog(SaveErrorLogInTextFile);
        ...
        public ResponseModel GetDynamicMessage(ApiFilterModel apiRequestMode, string baseURL)
        {
            // apiRequestMode.Module = "CFG";
            ResponseModel returnStatus = new ResponseModel(Core.Enums.URM.Status.Alert);
        ...
        public void DeleteErrorLog(string fileName, string flag = "")
        {
            try
            {
                        ...
                        if (subdirectories.Length > 0)
                        {
                            foreach (string subdirectory in subdirectories)
                            {
                                foreach (var file in Directory.GetFiles(subdirectory))
                                {
                                    ...
                                    if (string.IsNullOrEmpty(fileName))
                                    {
                                        //Note:****For Using Scheduler call every day                                    
                                        if (fileInfo.CreationTime >= DateTime.Now.AddDays(-10) || fileInfo.LastWriteTime >= DateTime.Now.AddDays(-10))
                                        {
                                    ...
                                    {
                                        //Note:****Redirect to page for Manual delete 
