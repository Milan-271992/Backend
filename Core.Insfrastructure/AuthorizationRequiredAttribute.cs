    ...
    public class AuthorizationRequiredAttribute: Attribute, IAuthorizationFilter
    {
        ...
        public void OnAuthorization(AuthorizationFilterContext filterContext)
        {
            if (filterContext.HttpContext.Request.Headers.Keys.Contains(AuthKey,StringComparer.OrdinalIgnoreCase) && filterContext.HttpContext.Request.Headers.Keys.Contains(AuthId, StringComparer.OrdinalIgnoreCase))
            {
                ...
                if (filterContext.HttpContext.Request.Headers.Keys.Contains("X-Request-Id", StringComparer.OrdinalIgnoreCase))
                {
                    
                    string x_request_id = filterContext.HttpContext.Request.Headers["X-Request-Id"].ToString().ToLower();
                    string cacheMemoryKey = EnumUtility.GetDescription(CacheKeysApi.UserSession);
                }
                ...
                if (plainAuthId.Contains("~"))
                {
                    ...
                    if (EncryptionUtility.GenerateAuthKey(filterContext.HttpContext.Request, loginLogId.ToString()) == AuthKeyValue)
                    {
                        string baseURL = configuration["ServiceURL:UPM"];
                        using (var client = new HttpClient())
                        {
                            ...
                            if (returnStatus.Status == Enums.URM.Status.Success)
                            {
